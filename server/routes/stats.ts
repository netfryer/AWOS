/**
 * Stats API routes - roles analytics.
 */

import type { Request, Response } from "express";
import { listDemoRunsForRoles } from "../../app/lib/demoRunsStore.js";
import type { RoleExecutionRecord } from "../../app/lib/demoRunsStore.js";
...
export async function rolesGet(req: Request, res: Response) {
  try {
    const hoursParam = (req.query.hours as string) ?? "168";
    const hours = hoursParam === "0" ? 0 : parseInt(hoursParam, 10) || 168;
    const limit = Math.min(
      5000,
      parseInt((req.query.limit as string) ?? "5000", 10) || 5000
    );
